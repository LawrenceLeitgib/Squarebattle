...
class Game {
  ...
  start() {
    if (
      ...
      if(this.musicOn)backgroundMusic.play();
  ...
  update(deltaTime) {
    ...
    if (isInside(this.mousePosition, rectMusic) && this.clickDown) {
      this.buyCount=0;
      if (this.musicOn) {
        this.musicOn=false;
        ...
        if (
          ...
          this.money >=
            this.addintionalCostRegen *
              (this.spaceShip.regenerationPourecent/2 + 0.1) *
              10
        ) {
          this.buyCount = 0;
          this.money -= Math.round(
            ...
            (this.spaceShip.regenerationPourecent/2 + 0.1) *
            10);
          this.spaceShip.regenerationPourecent += 0.2;
        }
        if (
