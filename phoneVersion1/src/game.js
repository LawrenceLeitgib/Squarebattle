...
canvas.addEventListener("touchstart", function (e) {
  ...
  xPosNew = Math.round(clientX - cRect.left);
  yPosNew = Math.round(clientY - cRect.top);
  inTheMainScreen=false
  if (xPosNew >= 0 && xPosNew <= 1000 && yPosNew >= 0 && yPosNew <= 695) {
    xPos = Math.round(clientX - cRect.left); // Subtract the 'left' of the canvas
    yPos = Math.round(clientY - cRect.top); // from the X/Y positions to make
    inTheMainScreen=true
  }
  if (e.touches.length >= 2) {
    clientX2 = e.touches[1].clientX;
    clientY2 = e.touches[1].clientY;
    xPosNew2 = Math.round(clientX2 - cRect.left);
    yPosNew2 = Math.round(clientY2 - cRect.top);
    inTheMainScreen=false

    if (xPosNew2 >= 0 && xPosNew2 <= 1000 && yPosNew2 >= 0 && yPosNew2 <= 695) {
      inTheMainScreen=true
      xPos = Math.round(clientX2 - cRect.left); // Subtract the 'left' of the canvas
      yPos = Math.round(clientY2 - cRect.top); // from the X/Y positions to make
    }
  }
});
canvas.addEventListener("touchmove", function (e) {
  ...
  yPosNew = Math.round(clientY - cRect.top);
  inTheMainScreen=false
  if (xPosNew >= 0 && xPosNew <= 1000 && yPosNew >= 0 && yPosNew <= window.innerHeight-200) {
    xPos = Math.round(clientX - cRect.left); // Subtract the 'left' of the canvas
    yPos = Math.round(clientY - cRect.top); // from the X/Y positions to make
    inTheMainScreen=true
  ...
  if (e.touches.length >= 2) {
    ...
    if (xPosNew2 >= 0 && xPosNew2 <= 1000 && yPosNew2 >= 0 && yPosNew2 <= window.innerHeight-200) {
      xPos = Math.round(clientX2 - cRect.left); // Subtract the 'left' of the canvas
      yPos = Math.round(clientY2 - cRect.top); // from the X/Y positions to make
      inTheMainScreen=true
...
class Game {
  constructor(gameWidth, gameHeight, ratio) {
    ...
    this.inTheMainScreen =inTheMainScreen
  ...
  update(deltaTime) {
    ...
    if(this.lives<=0 &&this.gamestate == GAMESTATE.RUNNING){
      xPos=0
      yPos=0
    ...
    //console.log(this.mousePosition)rectForTouchDown
    //console.log(rectForTouchDown)
  ...
  draw(context, context2) {
    [
      ...
      ...this.helpers,
      ...this.goldCoins,
    ].forEach((object) => object.draw(context, context2));
